    ...
    public static void main(String[] args) {
        while (true) {
            ...
            if (found) {
                ...
                System.out.println("Có giá " + "g[" + start_.toString() + ","
                        + end_.toString() + "] = " + end_.g_scores);
            } else {
    ...
    public static void Search(Node source, Node goal) {
        ...
        while ((!MO.isEmpty()) && (!found)) {
            ...
            for (Edge e : current.adjacencies) {
                ...
                // Nếu nút con đã được xét và g_score mới không nhỏ hơn g_score cũ thì bỏ qua ->
                // thực hiện lần lặp mới
                if ((DONG.contains(child)) &&
                        (temp_g_scores /* mới */ >= child.g_scores /* cũ */)) {
                    continue;
                }

                // Nếu nút con không có trong hàng đợi hoặc có g_score mới thấp hơn
                // Thì ta sẽ bắt đầu xét nút con đó (S = child)

                else if ((!MO.contains(child)) ||
                        (temp_g_scores < child.g_scores)) {

                    child.parent = current;
                    child.g_scores = temp_g_scores;

                    // Add node child vào tập MO
                    // (nếu node child có rùi -> cập nhật lại giá trị g_scores
