...
import gameCaro.bean.Setting;
import gameCaro.values.Value;
	...
	public static Setting LoadSettingInfo() {
		...
		if (!file.exists()) {
			try {
				...
				e1.printStackTrace();
				System.out.println("Có lỗi khi mở file setting.txt!");
	...
	public static void SaveSettingInfo(Setting setting) {
		...
		if (!file.exists()) {
			try {
				...
				System.out.println("File không tồn tại!. Đã tạo mới file setting.txt!");
			}
		}
		try {
			// Save data unicode
...
//			writer.write(color + "\n");
			...
			System.out.println("Có lỗi khi lưu file setting.txt!");
		}
	}
}
